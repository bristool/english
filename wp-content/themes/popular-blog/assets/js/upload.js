...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Popular_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    popular_blog_file_frame = wp.media.frames.popular_blog_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Popular_Blog_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    popular_blog_file_frame.on( 'select', function() {

      var state = popular_blog_file_frame.state('author-info-insert-image');
